...
ONLINE_CRITERIA = {
    ...
    "任务一：场景语义解析": {
        "max": 12,
        "description": "大模型接收任务卡1识别结果，正确理解场景语义并输出解析文本。共一张图六个物品，按识别数目打分",
        "module": "一、大模型智能体解析",
        "score_range": "0~12分（每个物品正确识别0~2分，共6个物品）",
    },
    "任务二：指令提取与步骤输出": {
        "max": 12,
        "description": "大模型精准提取任务卡2指令顺序，输出规范中文步骤及触发字符。共六步骤，按识别步骤打分",
        "module": "一、大模型智能体解析",
    ...
    "视觉流程运行状态": {
        ...
        "module": "二、VM视觉流程与数据协议",
        "score_range": "0~10分（流程流畅无报错、展示接收数据10分，有瑕疵酌情扣分）",
    },
    "固定数据格式与数值输出": {
        "max": 15,
        "description": "VM运行完毕，界面明确显示发送了协议表规定的#0;X;Y;RZ数据。格式错或数值伪造扣分。共六步骤，按识别步骤打分",
        "module": "二、VM视觉流程与数据协议",
        "score_range": "0~15分（每步骤0~2.5分，共6步骤）",
    },
    # 三、机器人仿真动作与闭环 (25分)
    "数据接收与代码响应": {
        "max": 10,
        ...
        "module": "三、机器人仿真动作与闭环",
        "score_range": "0~10分（日志区实时显示接收数据10分，缺失酌情扣分）",
    },
    "初始位姿偏移与复位": {
        "max": 15,
        "description": "机器人仿真软件主页3D画面中，机械臂基于固定初始位姿偏移执行动作，有明确抓取放置动作；每步后回归初始位姿再执行下一步。共六步骤，按识别步骤打分",
        "module": "三、机器人仿真动作与闭环",
        "score_range": "0~15分（每步骤0~2.5分，共6步骤）",
    },
    # 四、资料源码与答辩 (20分)
    "线上答辩表现": {
        "max": 20,
        "description": "PPT重点讲解大模型解析、VM数据驱动机械臂、语音方案设计；答辩流畅，时间把控精准",
        "module": "四、资料源码与答辩",
        "score_range": "0~20分（内容完整表达流畅20分，一般10~15分，较差0~9分）",
    },
}
...
ONLINE_DEDUCTIONS = {
    ...
    "VM发送数据格式不符": {
        "deduct": 8,
        "description": "VM发送数据格式不含#前缀，或X,Y,RZ数值与协议表规定不符（扣除8分）",
    },
    "机械臂未按规定位姿": {
        "deduct": 10,
        "description": "机械臂未从规定初始位姿出发，或每步执行后未回到初始位姿（扣除10分）",
    },
    "机器人仿真动画不直观": {
        "deduct": 5,
        "description": "机器人仿真动画展示不直观（扣除5分）",
    },
    "屏幕共享未设置时钟": {
        "deduct": 5,
        "description": "仿真演示环节屏幕共享没有设置时钟（扣除5分）",
    },
}
